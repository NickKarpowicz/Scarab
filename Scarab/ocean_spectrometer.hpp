...
class OceanSpectrometer : public Spectrometer {
  public:
    ...
    OceanSpectrometer(long device_id_input)
        : Spectrometer(odapi_get_formatted_spectrum_length(
              open_ocean_spectrometer_wrapper(device_id_input, &error),
              &error)),
          device_id(device_id_input) {
        is_initialized = (error == 0);
        if(is_initialized)
            odapi_get_wavelengths(device_id, &error, wavelengths_buffer.data(), pixel_count);
        // Get the device name
...
inline std::string
open_ocean_spectrometers(std::vector<std::unique_ptr<Spectrometer>> &spectrometer_set) {
    std::string output;
    ...
    for(int i = 0; i < device_id_count; i++) {
        ...
        if(((OceanSpectrometer *)spectrometer_set.back().get())->error) {
            output += std::format("Opening spectrometer {} failed with error {}\n",
                                  i,
                                  ((OceanSpectrometer *)spectrometer_set.back().get())->error);
        }
    }
    return output;
};
