...
class SpectralInterferometry {
    ...
    void filtered_hilbert(std::vector<double> &inData,
        ...
        pocketfft::r2c(pocketfft::shape_t{inData.size()},
        ...
        for(size_t i = 0; i < (num_freqs / 2 + 1); i++) {
            hilbert_time_buffer[i] *= std::exp(
                -std::pow((static_cast<double>(i) * dt - filter0) / filterSigma, filterOrd) /
                sqrt(2.0));
            hilbert_time_buffer2[i] = ii * hilbert_time_buffer[i];
        }

        pocketfft::c2r(pocketfft::shape_t{outDataReal.size()},
                       pocketfft::stride_t{static_cast<ptrdiff_t>(sizeof(std::complex<double>))},
                       pocketfft::stride_t{static_cast<ptrdiff_t>(sizeof(double))},
                       pocketfft::shape_t{0},
                       pocketfft::BACKWARD,
  ...
  public:
    ...
    void set_averaging(bool newState) { use_averaging = newState; }
    void reset_frequencies(size_t N, double fMin, double fMax) {
        if(N < 2)
            return;
        if(num_freqs == N && minF == fMin && maxF == fMax)
            return;
    ...
    void generate_time_plot() {
        ...
        for(size_t i = 0; i < (num_freqs / 2 + 1); i++) {
            ...
            time_filter[i] = std::exp(
                -std::pow((static_cast<double>(i) * dt - filterT0) / filter_width, filter_order) /
                sqrt(2.0));
    ...
    void acquire_reference_A(BatchAcquisition &batchControl,
                             const size_t N,
    ...
    void acquireReferenceB(BatchAcquisition &batchControl,
                           const size_t N,
                           const double integration_time,
                           const double seconds_to_wait,
                           Spectrometer &s) {
