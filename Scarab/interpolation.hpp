...
constexpr std::vector<double> fornberg_stencil(const size_t order,
                                               const std::span<const double> &positions,
                                               const double position_out) {
    ...
    for(size_t n = 1; n < n_pos; n++) {
        ...
        for(size_t v = 0; v < n; v++) {
            ...
            for(size_t m = 0; m <= min_n_order; m++) {
                double last_element =
                    m == 0 ? 0.0 : static_cast<double>(m) * delta_previous[(m - 1) * n_pos + v];
        ...
        for(size_t m = 0; m <= min_n_order; m++) {
            double first_element =
                m == 0 ? 0.0 : static_cast<double>(m) * delta_previous[(m - 1) * n_pos + n - 1];

            delta_current[m * n_pos + n] =
                (c1 / c2) * (first_element -
    ...
    return std::vector<double>(delta_current.begin() + order * n_pos,
                               delta_current.begin() + cols * n_pos);
}

constexpr inline double interp_fornberg(const double target_frequency,
                                        const std::span<const double> frequencies,
                                        const std::span<const double> wavelengths,
                                        const std::span<const double> spectrum_in) {
    ...
    size_t stencil_x_start =
        std::distance(wavelengths.begin(),
                      std::lower_bound(wavelengths.begin(), wavelengths.end(), target_wavelength));
    if(stencil_x_start < stencil_offset) {
        ...
        if(std::abs(wavelengths[stencil_x_start] - target_wavelength) >
           std::abs(wavelengths[stencil_x_start + 1] - target_wavelength)) {
            stencil_x_start++;
        }
        stencil_x_start -= stencil_offset;
    }
    std::span<const double> stencil_wavelengths =
        std::span(wavelengths).subspan(stencil_x_start, stencil_width);
...
inline std::vector<double> wavelength_to_frequency(const std::span<const double> frequencies,
                                                   const std::span<const double> wavelengths,
                                                   const std::span<const double> spectrum_in) {
